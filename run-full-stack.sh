...
# Run A-Proxy with Persona Service API in a development environment
# This script ensures proper configuration and database setup
...
DB_PATH="${PERSONA_SERVICE_DIR}/instance/persona_service.db"
...
if [ ! -f "$DB_PATH" ]; then
    echo -e "${YELLOW}Database not found. Initializing it now...${NC}"
    python3 init_db.py
else
    echo -e "${GREEN}Using existing database at: ${DB_PATH}${NC}"
fi
...
start_persona_service() {
  ...
  export DATABASE_URI="sqlite:///${DB_PATH}"
...
# Main execution
echo -e "${YELLOW}==== Setting up Full A-Proxy Development Stack ====${NC}"
